...
async def investigate(account_id: str, hop_radius: int = 2, time_window_days: int = 30, file: UploadFile = File(None)):
    ...
    phase3_path = BASE_DIR / "data/processed/phase3_risk_results.json"
    if os.path.exists(phase3_path):
        ...
        for state in phase3_results:
            if state.get("account_id") == account_id:
                ...
                return {**report, "subgraph": subgraph, "features": features, "flagged_transaction_id": state.get("transaction_id")}
    ...
    if file:
        ...
        tmp_path = BASE_DIR / "data/processed/phase1_full_results.csv"
        if not os.path.exists(tmp_path):
            raise HTTPException(status_code=400, detail="Default dataset not found and no file uploaded.")
            
    state = initial_state(str(tmp_path), account_id, hop_radius, time_window_days)
    result = cast(Any, aml_pipeline).invoke(input=state)
    if result.get("errors"):
        raise HTTPException(status_code=500, detail=result["errors"])
    report = result.get("final_report", {})
    subgraph = result.get("subgraph", {"nodes": [], "edges": []})
    return {**report, "subgraph": subgraph, "features": result.get("features", {}), "flagged_transaction_id": result.get("transaction_id")}
...
async def investigate_transaction(transaction_id: str):
    ...
    raise HTTPException(status_code=404, detail="Transaction ID not found in pre-computed results.")
